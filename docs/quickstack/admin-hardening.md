...
- [x] Verify: deleting a log on the VM does not delete it from GCS. *Verified 2026-07-16: first hourly batch landed (`auth_log/2026/07/16/…json`); deletion attempt rejected with explicit 403 — "subject to bucket's retention policy … cannot be deleted or overwritten until 2027-08-19". VM-side deletion is architecturally irrelevant (sink copies out of Cloud Logging, not off the VM).*
...
- [x] Perform at least one full restore drill from GCS backups to a fresh VM. Document the time-to-recover. *Drilled 2026-07-16 (whole drill incl. teardown: 16:14:40–16:21:39 UTC):*
  ...
  - ***Gap found and closed same day:** `/etc/nanormm/*.env` (bridge API key, tripwire webhook + DSN) was not in the app backup. Backup wrapper (`~/bin/rmm-gcs-backup.sh` on the VM, mirrored at `nanormm/ops/rmm-gcs-backup.sh`) now appends `./nanormm/etc-nanormm.tar.gz` to each archive; verified present in the GCS copy via a full end-to-end run.*
